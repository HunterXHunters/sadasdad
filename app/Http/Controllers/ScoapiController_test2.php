...
class ScoapiController_test2 extends BaseController 
{
...
//					curl_setopt($curl, CURLOPT_HTTPHEADER,$headers);
	...
	public function getErpObjectResponse()
	{
		...
		{	
				...
				$FIFOstorageLocation_ids=array();
				$fifo_products=array();
				$inputData=array('module_id' => $this->_request->input('module_id'),'access_token' => $this->_request->input('access_token'), 'plant_id' => $plantId,'type' => $objectType,'object_id' => $objectId,'action' => $action,'cancelled'=>$cancelled);


//echo "<pre/>";print_r($inputData);exit;

				$req = Request::create('scoapi_test/notifyEseal', 'POST',$inputData);
...
//					$value['pallet_capacity_ea']=$convt->getUom($value['product_id'],1,'pal');
				...
				if(count($returnOrders)>0){
					...
					return json_encode(['Status'=>1,'Message'=>'Server: '.$message,'Data'=>$returnOrders,'docNo'=>$docNo,'conversions'=>$conversions,'loc_erp_code'=>$plantId,'Price_lot_master'=>$price_lot_master,'Sku_Master'=>$sku_master]);
				} else 					
		...
		return json_encode(['Status'=>1,'Message'=>'Data inserted succesfully','Data'=>$delivery]);
			...
			case "Cancelled";
						//echo "hai";exit;
						$delivery=DB::table('delivery_master as dm')->leftJoin('locations as l1','l1.location_id','=','dm.frm_location')->leftJoin('locations as l2','l2.location_id','=','dm.to_location');
						$delivery=$delivery->where('document_no',$this->_request->input('object_id'))->where('action_code','=',5)->get(['dm.id as id','dm.document_no','dm.frm_location','dm.to_location','dm.receving_location','dm.is_sto','dm.manufacturer_id','dm.user_id','dm.shipment_no','dm.delivery_shipment_flag','dm.doc_date','dm.type','dm.sto_no','dm.is_processed','l1.location_name as frm_location_name','dm.action_code as action_code','l2.location_name as to_location_name'])->toArray();
						$delivery=json_decode(json_encode($delivery[0]),true);
				...
				$tp_no=DB::table('tp_attributes as ta')->where('ta.value',$this->_request->input('object_id'))->get(['tp_id']);
				 // echo "<pre/>hhh";print_r($tp_no);exit;
				...
				// $iots1[]=$iots;
				// foreach ($iots as $iot ) {
				$eseal_ids=DB::table('eseal_'.$mfg_id.' as e')->whereIn('e.primary_id',explode(',',$kkk))->get(['primary_id','batch_no','pkg_qty'])->toArray();
				// }
...
//echo "<pre/>";print_r($eseal_ids);exit;
			...
			$packed_po=DB::table('eseal_'.$mfg_id.' as e')
				...
				return json_encode(['Status'=>1,'Message'=>'Packed PO Retrieved Succesfully','Data'=>$packed_po]);
					...
					return json_encode(['Status'=>0,'Message'=>'Server:No PO packed against material code.']);
		...
		return json_encode(['Status'=>1,'Message'=>'Data inserted succesfully','Data'=>$delivery]);
				...
				case "STOCKTRANSFER":
					...
					$idbNumber = $this->_request->input('idb_number');
					$poIdbFlag = $this->_request->input('po_idb_flag');
					if(!$poIdbFlag)
					{
					...
					$params = 'po_number='.$poNumber.'&idb_number='.$idbNumber;
					$body=0;
					$methodType='GET';
					...
					$importPoInsert = [];
					foreach ($result->po_details as $key => $value) {
						$value->material = ltrim($value->material, '0');
						...
						if (!$impotID)
						{
							...
							$importPoInsert[] = [
								...
								'po_item'		=> $value->po_item,
								'idb_number'	=> $value->idb_no,
								'idb_item'		=> $value->idb_item,
								'material'		=> $value->material,
								'material_des'	=> $value->material_dec,
								'product_id'	=> $product_id,
								'location_id'	=> '',
								'vender'		=> $value->vendor,
								'open_quantity'	=> $value->open_quantity,
								'order_uom'		=> $value->order_uom,
								'base_uom'		=> $value->base_uom,
...
public function BindWithTrackupdate()
{
	...
	try
	{
		...
		DB::beginTransaction();
		$req = Request::create('scoapi_test/BindEseals1', 'POST');
		$res = app()->handle($req);
		...
		if($res1->Status)
		{
			$request = Request::create('scoapi_test/UpdateTracking', 'POST');
			$res = app()->handle($request);
...
//		   	$qty=$convt->getUom($poDetails->product_id,$poDetails->qty,$poDetails->order_uom); 
			...
			// $pallets_to_pack=$convet->getUomAll($po_qty,$order_uom,$poDetails->product_id);
           ...
           /*$packed_qty_eaches = DB::table('conversions as c')->join('production_orders as po','c.product_id','=','po.product_id')
			  ...
			  $packed_qty_EA= $bindQty = DB::table('eseal_'.$mfgId)->where(['po_number'=>$po_number,'level_id'=>0,'is_active'=>1])
              ->sum('pkg_qty');
			  $packed_qty_z01=$convet->getZ01toEA($packed_qty_EA,$uom,$p_id);
			  // $packed_qty_EA=$convet->getPALtoEA($packed_qty_pallets,$p_id);
			    // echo $packed_qty_z01;exit;
	    ...
	    /*$packed_qty_pallets = DB::table('conversions as c')->join('production_orders as po','c.product_id','=','po.product_id')
	    			...
	    			// $palletdivision = ceil($packed_qty_eaches[0]->packed_qty_eaches/$packed_qty_pallets[0]->base_quantity);
...
/////////////////////////////////end of Level0,Level1,Level2 Info//////////////////////////////////////
			...
			if(!empty($primaryCollection)){
				...
				$final['trace_info'] = $trackfinal;
				 ...
				 $pids = DB::select('select cast(cast(pkg_qty as UNSIGNED) as char) as qty,level_id as level,cast(primary_id as char) as primary_id,products.name,CAST(pid as CHAR) as pid,eseal_'.$manufacturer_id.'.mrp,eseal_'.$manufacturer_id.'.batch_no from eseal_'.$manufacturer_id.' join products on products.product_id = eseal_'.$manufacturer_id.'.pid where primary_id IN (' .  $levelIds. ')')->toArray();		                
echo"hii";print_r($pids);exit;
	   ...
	   //Log::info(__LINE__);
		...
		try{
			...
			if(!empty($res)){
				...
				$attributeMaps =  DB::table('bind_history')->whereIn('eseal_id',$res)->distinct()->value('attribute_map_id');
				//echo($attributeMaps); exit;
				$componentIds =  DB::table('attribute_mapping')->whereIn('attribute_map_id',$attributeMaps)->where('attribute_name','Stator')->get(['value']);
