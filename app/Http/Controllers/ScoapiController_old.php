...
class ScoapiController_old extends BaseController 
{
...
//					curl_setopt($curl, CURLOPT_HTTPHEADER,$headers);
	...
	public function getErpObjectResponse()
	{
		...
		{	
				...
				$FIFOstorageLocation_ids=array();
				$fifo_products=array();
				$inputData=array('module_id' => $this->_request->input('module_id'),'access_token' => $this->_request->input('access_token'), 'plant_id' => $plantId,'type' => $objectType,'object_id' => $objectId,'action' => $action,'cancelled'=>$cancelled, 'doc_number' => $docNumber, 'is_po_number' => $isPoNumber);
...
public function notifyEseal()
{
	...
	$this->erp=new ConnectErp($mfg_id);
		...
		if($objectType!='get_po_orders' && $objectType!='create_po_order' && $objectType!='PORDER' && $objectType!='putaway' &&$objectType!='Cancelled'){		
		...
		try
	{
			...
			{	
				...
				$esealOrders=DB::table('production_orders as po')
							...
							->where('po.is_erp',0)
							->where('po.is_eseal',1)
							->get(['po.product_id','po.location_id',DB::raw('IF(po.erp_doc_no = 0, po.eseal_doc_no, po.erp_doc_no) as order_no'),'po.order_uom','po.qty as poqty','po.is_eseal','l.location_name as loc_name','p.name as product_name','p.material_code','l.erp_code','p.ean as ean'])->toarray();
...
//					$value['pallet_capacity_ea']=$convt->getUom($value['product_id'],1,'pal');
				...
				if(count($returnOrders)>0){
					...
					return json_encode(['Status'=>1,'Message'=>'Server: '.$message,'Data'=>$returnOrders,'docNo'=>$docNo,'conversions'=>$conversions,'loc_erp_code'=>$plantId,'Price_lot_master'=>$price_lot_master]);
				} else 					
					return json_encode(['Status'=>0,'Message'=>'Server: No orders found']);
		...
		/*			$poDetails=DB::table('production_orders')->where('erp_doc_no',$po_number)->orWhere('eseal_doc_no',$po_number)->get()->toarray(); */
				...
				if(count($returnOrders)>0){
					$message="Order retrieved successfully";
					return json_encode(['Status'=>1,'Message'=>'Server: '.$message,'Data'=>$returnOrders,'conversions'=>$conversions]);
				} else 					
					return json_encode(['Status'=>0,'Message'=>'Server: No orders found']);
				break;
		...
		return json_encode(['Status'=>1,'Message'=>'Data inserted succesfully','Data'=>$delivery]);
					break;
						case "Cancelled";
						$delivery=DB::table('delivery_master as dm')->leftJoin('locations as l1','l1.location_id','=','dm.frm_location')->leftJoin('locations as l2','l2.location_id','=','dm.to_location');
						$delivery=$delivery->where('document_no',$this->_request->input('object_id'))->where('action_code','=',5)->select('dm.id','dm.document_no','dm.frm_location','dm.to_location','dm.receving_location','dm.is_sto','dm.manufacturer_id','dm.user_id','dm.shipment_no','dm.delivery_shipment_flag','dm.doc_date','dm.type','dm.sto_no','dm.is_processed','l1.location_name as frm_location_name','dm.action_code as action_code','l2.location_name as to_location_name')->get()->toArray();
		...
		if($action=='delivery'){
				//where('is_sto',0)->
				$delivery=DB::table('delivery_master as dm')->leftJoin('locations as l1','l1.location_id','=','dm.frm_location')->leftJoin('locations as l2','l2.location_id','=','dm.to_location')->where('action_code','>=',4)->where('document_no',$this->_request->input('object_id'))->get(['dm.id','dm.document_no','dm.frm_location','dm.to_location','dm.receving_location','dm.is_sto','dm.manufacturer_id','dm.user_id','dm.shipment_no','dm.delivery_shipment_flag','dm.doc_date','dm.type','dm.sto_no','dm.is_processed','l1.location_name as frm_location_name','l2.location_name as to_location_name'])->toArray();
				if(count($delivery)){
				$delivery=json_decode(json_encode($delivery[0]),true);	
				} else {
					throw new Exception('Delivery Not available');
				}
				$delivery['items']=DB::table('delivery_details')->where('ref_id',$delivery['id'])->get()->toArray();
		return json_encode(['Status'=>1,'Message'=>'Data inserted succesfully','Data'=>$delivery]);
		} else {
				...
				case "STOCKTRANSFER":
					...
					case "mulTest": 
					return json_encode(['Status'=>1,'Message'=>'Server: mulTest']);
					break;
					...
					$methodType='GET';
					$result=$this->erp->request($method,'',$bodyData,$methodType);
					// return json_encode(['Status'=>$status,'Message'=>'Server: Working fine', 'Data' => json_encode($result)]);
					$result=json_decode($result);
					if (empty($result)){
					...
					return json_encode(['Status'=>1,
						'Message'		=>'Server: '.$result->EStatus->Message,
						'po_number'		=> ($poFlag == true ? $docNumber : ''),
						'idb_number'	=> ($poFlag == false ? $docNumber : ''),
						'vendor'		=> $result->EPodetails->item[0]->Vendor,
						'Data'			=> $finalData]);
					// return json_encode(['Status'=>1,'Message'=>'Server: '.$result->message, 'Data' => $result->po_details]);
					break;
				...
				if(!empty($level) && $level >0 && !empty($po_number))
				{
				   ...
				   {    
				   	$poDetails=DB::table('production_orders')->where(function($query) use ($po_number){
						...
						$req = Request::create('scoapi/bindMapWithEseals', 'POST',$inputData);
						$originalInput=$this->_request->all();
						$this->_request->replace($req->all());
...
public function getPoQuantity_old(){
	try{
		$status =1;
...
//		   	$qty=$convt->getUom($poDetails->product_id,$poDetails->qty,$poDetails->order_uom); 
    ...
    log::info(['Status'=>$status,'Message'=>$message,'qty'=>$qty,'packedqty'=>$bindQty,'material_code'=>$material_code,'description'=>$description,'confirmQty'=>$confirmQty,'eSealCnfQty'=>$eSealCnfQty]);
	return json_encode(['Status'=>$status,'Message'=>$message,'qty'=>$qty,'po_qty'=>$po_qty,'packedqty'=>(int)$bindQty,'material_code'=>$material_code,'description'=>$description,'confirmQty'=>$confirmQty,'eSealCnfQty'=>$eSealCnfQty]);
}
	...
	public function confirmProductionOrder()
	{
		...
		try{ 
			...
			if(!$orderID){
				...
				$insert=DB::table('po_confirm_queue')->insertGetId(['po_number'=>$productionOrderId,'mfg_id'=>$manufacturerId,'qty'=>$serialsArray,'userstamp'=>$userId,'final_confirm'=>$conf_status,'remarks'=>$remarks,'cost_loss'=>$quality_cost_loss,'conf_date'=>$mfg_date]);
	...
	public function confirmProductionOrderEcc()
	{
        $startTime = $this->getTime();
        $status=0;
        $message=0;
...
/*			echo "<pre>";
			...
			foreach ($am_ids as $akey => $avalue) {
				...
				$temp=array('sku_info'=>$sku_info,'price_lot'=>$price_lot,'id'=>$am_id,'shift'=>$shift);
				// print_r( $temp);exit;
				$exists=0;
			...
			foreach ($amc as $key => $value) {
				...
				//				echo $c_uomQty; exit;
					...
					$inputArray['order_number']=$poDetails->erp_doc_no;
					...
					*/
					...
					$inputArray['final_confirmation_indc']=$_podetails->fina1_confirm==1?"X":" ";

					$inputArray['posting_date']=$poDetails->conf_date;
					$inputArray['quality_cost_loss']=$poDetails->cost_loss;
...
private function mapTPAttributes($codes, $esealTable, $srcLocationId, $tpDataMapping, $transitionTime){
  ...
  try{
	...
	public function deliveryCancel_Confirmation(){
		...
		if(count($tp_info)>1)
			throw new Exception("tomany tracks exist agains to this TP", 1);
		if(count($tp_info)==0)
			throw new Exception("no tracks found agains to this tp", 1);
           ...
           if($deliveryId){
           		if($input['action_code']<=4)
           			throw new Exception("delivery already exist, Plese consult team for further", 1);
           			 ...
           			 $deliveryId=DB::table('delivery_master')->where('action_code','>',4)->where('document_no',$input['document_no'])->value('id');
           			 if($deliveryId){
           				throw new Exception("cencelDelivery already exist, Plese consult team for further", 1);
           			 }else{
...
exit;
	*/
	...
	$body=array('headerData'=>array('obd_number'=>$deliveryNo,'po_sto_number'=>$sto),'itemData'=>$eccItemData);
...
public function grnCanToConfirmation(){
	...
	public function putawayConfirmation(){
		...
		try{
			...
			if(count($queue)<=0)
				throw new Exception(" document_no not found in database", 1);
			$baseQ=$queue[0];
			$itemData=[];
	...
	function grnCreation(){
		...
		foreach ($params['itemData'] as $key => $value) {
			...
			if(empty($esealBankData)){
				...
				$output['Message'] = 'Server: IOTs are not available in eSeal';
				return $output;
			}
			...
			if (count($explodeIds) != count($esealBankData))
			{
				...
				$output['Message'] = 'Server: Some of the codes are not available in eSeal';
				return $output;
			}
			...
			if(!$plantId){
				...
				$output['Message'] = 'Server: Storage Location is not available in eSeal';
				return $output;
			}
			...
			// $result = array();
			if (empty($result)){
				return json_encode(Array('Status'=> 0, 'Message' =>'Server: Please try after sometime'));
				$output['Message'] = 'Server: Please try after sometime';
			return $output;
			...
			if($result->Yh3mmEsealGrnCreationResponse->EStatus->Status == 1){
				$data = $result->Yh3mmEsealGrnCreationResponse->EFinal->item;
				$importPODetails = DB::table('ImportPO')
									->where('po_number', $poNumber)
									->where('material', $value['material'])
									->first();
				$grnNumber = $result->Yh3mmEsealGrnCreationResponse->EFinal->item[0]->GrnNumber;
				$insertImportGrn = array(
					'import_po_id'	 => $importPODetails->id,
					'po_number'		 => $result->Yh3mmEsealGrnCreationResponse->EFinal->item[0]->Ponumber,
					'ibd_number'	 => $result->Yh3mmEsealGrnCreationResponse->EFinal->item[0]->Ibdnumber,
					'open_quantity'	 => '',
					'action_quantity'=> $grnQty,
					'status'		 =>	1,
					'batch'			 => $result->Yh3mmEsealGrnCreationResponse->EFinal->item[0]->Batch,
			...
			foreach ($value['scandata'] as $skey => $svalue) {
				...
				$attributes = json_encode(array('material_code' => $value['material'], 'po_number' => $docNumber, 'MFG_DATE' => date('Y-m-d'), 'sku_info' => $skuCode, 'uom' => $value['uom'], 'grn_number' => $grnNumber));
		...
		return json_encode(Array('Status'=> 1, 'Message' =>'Server: '.$message, 'Data' => $data));
		return json_encode(['Status'=>1,'Message'=>'Server: '.$message,'Data'=>$data]);
	}
...
//echo "<pre/>";print_r($manf_date1);exit;
           ...
           $packed_qty_eaches = DB::table('conversions as c')->join('production_orders as po','c.product_id','=','po.product_id')
	    					->where('po.product_id','=',$poDetails->product_id)
	    					->where('c.alt_uom','=',$poDetails->order_uom)
	    					->where('c.product_id','=',$poDetails->product_id)
	    					->where('po.erp_doc_no','=',$po_number)
	    					->orWhere('po.eseal_doc_no','=',$po_number)
	    					->select(DB::raw(''.$bindQty.'*c.base_quantity AS packed_qty_eaches'))
	    					->get()->toarray();

	    $packed_qty_pallets = DB::table('conversions as c')->join('production_orders as po','c.product_id','=','po.product_id')
	    					->where('po.product_id','=',$poDetails->product_id)
	    					->where('c.alt_uom','=','PAL')
	    					->where('c.product_id','=',$poDetails->product_id)
	    					->where('po.erp_doc_no','=',$po_number)
	    					->get('base_quantity')->toarray();
			  
	    			$palletdivision = $packed_qty_eaches[0]->packed_qty_eaches/$packed_qty_pallets[0]->base_quantity;

	    			//echo $palletdivision;exit;
    ...
    log::info(['Status'=>$status,'Message'=>$message,'qty'=>$qty,'packedqty'=>$bindQty,'material_code'=>$material_code,'description'=>$description,'confirmQty'=>$confirmQty,'eSealCnfQty'=>$eSealCnfQty,'EAN'=>$ean]);
	return json_encode(['Status'=>$status,'Message'=>$message,'qty'=>$qty,'po_qty'=>$po_qty,'packedqty'=>(int)$bindQty,'material_code'=>$material_code,'description'=>$description,'confirmQty'=>$confirmQty,'eSealCnfQty'=>$eSealCnfQty,'EAN'=>$ean,'mfg_date'=>$manf_date,
		'Qty_in_eaches'=>$kk1,'packed_Qty_in_eaches'=>$packed_qty_eaches[0]->packed_qty_eaches,'packed_Qty_in_pallets'=>$palletdivision,'order_uom'=>$order_uom[0]->order_uom]);
}
